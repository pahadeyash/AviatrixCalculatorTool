...
function dx_port_output_script(f){
  switch (f.bandwidth.value){
    case "50":
      f.dx_port.value = (0.03 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "100":
      f.dx_port.value = (0.06 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "200":
      f.dx_port.value = (0.12 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "300":
      f.dx_port.value = (0.18 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "400":
      f.dx_port.value = (0.24 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "500":
      f.dx_port.value = (0.30 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "1000":
      f.dx_port.value = (0.30 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
    case "10000":
      f.dx_port.value = (2.25 * 24 * 30 * document.querySelectorAll('input[name=regions]:checked').length * Number(f.number_vpc.value)).toFixed(2);
      break;
  }
...
function dx_out_output_script(f){
    f.dx_out.value = ((f.bandwidth.value * 60 * 60 * 24 * 365 * 0.03) / (12 * 8 * 1024)).toFixed(2);
    f.dx_out_year.value = ((f.bandwidth.value * 60 * 60 * 24 * 365 * 0.03) /  (8 * 1024)).toFixed(2);
}

function avx_out_script(f){
  var bandwidth_GBm = (f.bandwidth.value * 60 * 60 * 24 * 365) / (12 * 8 * 1024);
  if (bandwidth_GBm<=1){
      f.avx_out.value = 0;
  }
...
function avx_instance_script(f){
  switch (f.bandwidth.value){
    case "50":
      f.avx_instance_type.value = "t2.micro";
    ...
    case "100":
      ...
      break;
    case "200":
      f.avx_instance_type.value = "m4.xlarge";
    ...
    case "500":
      ...
      break;
    case "1000":
      f.avx_instance_type.value = "m4.4xlarge";
...
function avx_calculate_total(f){
  f.avx_total.value = (Number(f.avx_instance.value) + Number(f.avx_licensing.value) + Number(f.avx_out.value) + Number(f.avx_dia.value)).toFixed(2);
  f.avx_total_year.value = (Number(f.avx_instance_year.value) + Number(f.avx_licensing_year.value) + Number(f.avx_out_year.value) + Number(f.avx_dia_year.value)).toFixed(2);
}
